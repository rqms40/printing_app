...
**Goal:** Add real-time chat to GRIDGO — customers chat with GridBot (AI), admin support, or their assigned rider; admins manage an inbox with assignment and close controls.
...
Invoke `frontend-design:frontend-design` with this brief:
> Build `ChatSelectScreen` for Flutter. Shows chat type selection cards. Receives optional `orderId` (int?) and `orderStatus` (String?) as constructor params. Dark/yellow theme (`#1A1A1A`, `#FFDE58`). Three cards: 🤖 GridBot ("Instant answers about GRIDGO"), 👨 Admin Support ("Chat with our team"), 🛵 My Rider ("Track & coordinate" — only shown when `orderId != null` and `orderStatus` is one of: `rider_assigned`, `picked_up`, `on_the_way`, `arrived_at_destination`). Tapping a card calls `ref.read(chatProvider.notifier).createConversation(type, orderId: orderId)` then navigates to `/customer/chat/:id` with the returned conversation ID. Uses `ConsumerStatefulWidget`.
...
- [ ] Rider option absent when order has no assigned rider
- [ ] Rider option visible on order with status `rider_assigned` or later
