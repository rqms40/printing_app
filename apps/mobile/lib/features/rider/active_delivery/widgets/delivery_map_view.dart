...
/// Rider's active delivery map with real road route and live position.
...
class _DeliveryMapViewState extends ConsumerState<DeliveryMapView> {
  ...
  Future<void> _loadRoute() async {
    ...
    setState(() {
      _routePoints = points;
      _isLoading = false;
      _riderIndex = (points.length * 0.3).round();
    });
    _emitRiderLocation();
    _startRidersimulation();
  }

  void _emitRiderLocation() {
    if (_routePoints.isEmpty) return;
    final point = _routePoints[_riderIndex];
  ...
  void _startRidersimulation() {
    _riderTimer = Timer.periodic(const Duration(seconds: 2), (timer) {
      ...
      setState(() {
        if (_riderIndex < _routePoints.length - 1) {
          _riderIndex++;
      ...
      if (shouldEmitLocation) _emitRiderLocation();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final riderPoint = _routePoints[_riderIndex];
