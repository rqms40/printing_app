...
content = content.replace(/dark:bg-white\/80 dark:bg-black\/60/g, 'dark:bg-black/60');
content = content.replace(/dark:bg-white\/95 dark:bg-black\/90/g, 'dark:bg-black/90');

// Remove isLogoGlassy completely by filtering out lines containing it or rect
const lines = content.split('\n');
const filteredLines = lines.filter(line => !line.includes('isLogoGlassy') && !line.includes('setIsLogoGlassy') && !line.includes('const rect ='));
...
content = content.replace(target1, target1 + overlay);
content = content.replace(target2, 'className="text-center max-w-2xl px-4 relative z-10"');
content = content.replace(target3, target3 + overlay);
content = content.replace(target4, 'className="max-w-6xl mx-auto px-6 relative z-10"');
content = content.replace(target5, target5 + overlay);
content = content.replace(target6, 'className="max-w-[1100px] mx-auto px-8 w-full relative z-10"');
content = content.replace(target7, target7 + overlay);
content = content.replace(target8, 'className="px-8 relative z-10"');

fs.writeFileSync('src/App.tsx', content);
console.log('App.tsx final fixes applied securely with line filtering (including rect)!');
