...
const versionedApkAssetName = 'GRIDGO-v1.12.4.apk';

export default function DownloadPage() {
  const latestApkUrl = getLatestApkDownloadUrl(undefined, latestApkAssetName);
  const versionedApkUrl = getVersionedApkDownloadUrl(
    currentReleaseVersion,
    undefined,
    versionedApkAssetName,
  );
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <section className="mt-10 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-transparent p-7 shadow-2xl md:p-12">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="inline-flex items-center gap-2 text-sm text-white/70"><BadgeCheck size={16} aria-hidden="true" /> Signed GitHub release</span>
          </div>
          ...
          <div className="mt-7 grid gap-10 md:grid-cols-[1.35fr_0.65fr] md:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-7 text-white/75 md:text-lg">
                Install the current Android beta to access GRIDGO from your phone. The app connects securely to the GRIDGO legacy domain and is distributed from its signed GitHub release.
              </p>
            </div>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            ...
            >
              <span><span className="block text-xs font-bold uppercase tracking-widest">Recommended</span><span className="mt-1 block text-xl font-black">Download latest APK</span><span className="mt-1 block text-sm font-medium">Always resolves to the newest release</span></span>
          ...
          <div className="mt-8 grid gap-4 rounded-2xl border border-white/10 bg-black/25 p-5 text-sm leading-6 text-white/70 md:grid-cols-[auto_1fr_auto] md:items-center">
            <Smartphone className="text-[var(--color-primary)]" aria-hidden="true" />
            <p><span className="font-bold text-white">First install?</span> Android may ask you to allow installs from your browser. Download only from this official page or the linked GitHub release, then open the APK to install.</p>
            <a href={releasePageUrl} className="inline-flex items-center gap-2 font-bold text-[var(--color-primary)] hover:text-white">View release notes <ExternalLink size={15} aria-hidden="true" /></a>
          </div>
          ...
          <div className="mt-5 flex items-start gap-3 text-sm text-white/55">
            ...
            <p>GRIDGO does not host APK files on the application server. GitHub Releases provides the immutable versioned download and a stable latest-download link.</p>
          </div>
        </section>
      </div>
    </main>
