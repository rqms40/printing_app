...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <Tag
      icon={isPaper ? <FileTextOutlined /> : <AppstoreOutlined />}
      style={{
        background: isPaper ? '#2A1F00' : '#001A2E',
        border: `1px solid ${isPaper ? '#FFDE58' : '#42A5F5'}`,
        color: isPaper ? '#FFDE58' : '#42A5F5',
        borderRadius: 6,
        fontSize: 11,
        fontWeight: 600,
      }}
    >
      {isPaper ? 'Paper' : '3D Print'}
    </Tag>
  );
}

/** Mini carousel card preview — mirrors the mobile card design */
function CardPreview({ card }: { card: DailyGridCard }) {
  const circleD = 64;
  ...
  return (
    <div style={{ position: 'relative', height: circleD, minWidth: 180 }}>
      ...
      >
        <div style={{ color: '#F0F0F0', fontWeight: 700, fontSize: 12, lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 100 }}>
          {card.title || 'Untitled'}
        </div>
        {card.subtitle && (
          <div style={{ color: '#666', fontSize: 10, marginTop: 2 }}>{card.subtitle}</div>
        )}
      </div>
      {/* Circle image */}
      <div
...
export function DailyGridList() {
  ...
  const openCreate = () => {
    ...
    setImagePreviewUrl('');
    form.resetFields();
    form.setFieldsValue({ category: 'paper', isActive: true, sortOrder: cards.length });
    setDrawerOpen(true);
  };
  ...
  const handleToggleActive = async (card: DailyGridCard) => {
    try {
      await apiClient.patch(`/daily-grid/admin/${card.id}`, { isActive: !card.isActive });
      void fetchCards();
    } catch {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    setCards(next);
    try {
      await apiClient.patch('/daily-grid/admin/reorder', { ids: next.map((c) => c.id) });
    } catch {
    ...
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <Text style={{ color: '#555', fontSize: 13 }}>
            {activeCount} active card{activeCount !== 1 ? 's' : ''} shown in the customer carousel
          </Text>
        </div>
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={openCreate}
          style={{ background: '#FFDE58', borderColor: '#FFDE58', color: '#141414', fontWeight: 700 }}
        >
          New Card
        </Button>
      </div>
      ...
      {cards.length === 0 && (
        <div style={{ ...S.card, padding: 48, textAlign: 'center' }}>
          <AppstoreOutlined style={{ fontSize: 48, color: '#333', marginBottom: 16 }} />
      ...
      {cards.map((card, index) => (
        ...
        >
          <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
              <Tooltip title="Move up">
                <Button
                  ...
                  disabled={index === 0}
                  style={{ background: '#1A1A1A', borderColor: '#2E2E2E', color: index === 0 ? '#333' : '#808080' }}
                />
              </Tooltip>
              <div style={{ textAlign: 'center', color: '#444', fontSize: 11, fontWeight: 700 }}>
                {index + 1}
              </div>
              <Tooltip title="Move down">
                <Button
                  ...
                  style={{ background: '#1A1A1A', borderColor: '#2E2E2E', color: index === cards.length - 1 ? '#333' : '#808080' }}
                />
              </Tooltip>
            </div>
            ...
            <Divider type="vertical" style={{ height: 72, borderColor: '#2A2A2A', margin: '0 4px' }} />

            {/* Details */}
            <div style={{ flex: 1, minWidth: 160 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                <Text strong style={{ color: '#F0F0F0', fontSize: 14 }}>{card.title}</Text>
                <CategoryBadge category={card.category} />
                {!card.isActive && (
                  <Tag style={{ background: '#1A1A1A', borderColor: '#333', color: '#555', borderRadius: 6, fontSize: 10 }}>
                    HIDDEN
                  </Tag>
                )}
              </div>
              {card.subtitle && (
                <Text style={{ color: '#666', fontSize: 12, display: 'block', marginBottom: 4 }}>
                  {card.subtitle}
                </Text>
              )}
              {card.imageUrl && (
                <Text style={{ color: '#444', fontSize: 11, display: 'block', maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {card.imageUrl}
                </Text>
              )}
            </div>
            ...
            <Space size={8} style={{ flexShrink: 0 }}>
              <Tooltip title={card.isActive ? 'Hide from carousel' : 'Show in carousel'}>
                <Button
                  size="small"
                  icon={card.isActive ? <EyeOutlined /> : <EyeInvisibleOutlined />}
              ...
              >
                <Button
                  ...
                  icon={<DeleteOutlined />}
                  danger
                  style={{ background: '#1A1A1A', borderColor: '#3A1A1A', color: '#EF5350' }}
                />
              </Popconfirm>
            </Space>
          </div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            label={<Text style={{ color: '#A0A0A0', fontSize: 12 }}>Subtitle</Text>}
            name="subtitle"
          >
          ...
          <Form.Item
            label={<Text style={{ color: '#A0A0A0', fontSize: 12 }}>Printing Category</Text>}
            name="category"
            rules={[{ required: true }]}
          >
            <Select
          ...
          <Form.Item
            ...
            name="sortOrder"
            help={<Text style={{ color: '#444', fontSize: 11 }}>Lower = appears first. Use arrows on the list to reorder.</Text>}
          >
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>

          <Form.Item
            label={<Text style={{ color: '#A0A0A0', fontSize: 12 }}>Active</Text>}
            name="isActive"
