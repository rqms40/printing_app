...
async function seed() {
  ...
  const [supplierProfile] = await typedQuery<IdRow>(
    ...
    [
      supplierUserId,
      'Davao Print Co',
      'Demo supplier for GRIDGO marketplace pilot — large format, apparel, and document printing.',
      '+639193234567',
      'supplier@gridgo.ph',
      'Quimpo Blvd, Ecoland, Davao City',
      '["Davao City","Toril","Calinan"]',
      // Pre-seed common focuses so settings UI has a baseline; supplier can re-rank.
  ...
  await ds.query(
    ...
    [
      supplierProfileId,
      superAdminId,
      'Seed pilot supplier — verified for demo marketplace flows',
    ],
  );

  // Leaf RFQ capabilities so matching can find the demo shop.
