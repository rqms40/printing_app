...
export class AddressesController {
  ...
  @Put(':id')
  updateAddress(@Request() req: any, @Param('id') id: number, @Body() dto: UpdateAddressDto) {
    return this.addressesService.update(id, req.user.sub, dto);
  }

  @Delete(':id')
  deleteAddress(@Request() req: any, @Param('id') id: number) {
    return this.addressesService.remove(id, req.user.sub);
  }

  @Patch(':id/default')
  setDefault(@Request() req: any, @Param('id') id: number) {
